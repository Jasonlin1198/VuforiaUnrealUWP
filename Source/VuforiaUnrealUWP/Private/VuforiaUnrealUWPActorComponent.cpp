...
#include "VuforiaUnrealUWPActorComponent.h"
...
void UVuforiaActorComponent::HandleAppResume()
{
	...
	// Executes bound OnCameraActive() function delegate set in Actor.cpp
...
void UVuforiaActorComponent::InitTrackerAndDataset()
{
	...
	//for (int tIdx = 0; tIdx < DataSet->getTrackables().size(); tIdx++)

	//{
	//	// Start Extended Tracking has been removed from the API - https://developer.vuforia.com/forum/unity/startextendedtracking-and-stopextendedtracking-issue-new-update
...
void UVuforiaActorComponent::DeinitTrackerAndDataset()
{
	...
	if (DataSet != nullptr)
	{
		//if (objectTracker->getActiveDataSets(0) == DataSet &&
